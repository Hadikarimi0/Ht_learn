...
def load_haj_hadi_from_json(file_name):
    with open(file_name, 'r', encoding='utf-8') as file:
        return json.load(file)

haj_hadi = load_haj_hadi_from_json('h.json')

def process_message(m):
    global haj_hadi
    if "/" in m.text:
        word = m.text
        if "! " in word and m.author_guid in Addmins:
            ...
            with open('h.json', 'w', encoding='utf-8') as file:
                json.dump(haj_hadi, file, ensure_ascii=False, indent=4)
     ...
     '''
			...
			if m.text == "قفل فایل خاموش":
				...
				if m.author_guid in Addmins:
					zed__link = False
					m.reply("ضد لینک خاموش شد.")
			if m.object_guid in group and onn_bot == True:
...
•┣➬ fuck [⇎]  فاک
•┗━━━━━━━━━━━•• 
و همچنین میتوان از دستورات سرگرمی مانند جوک و حدیث و فونت  و... استفاده کرد
                """)
