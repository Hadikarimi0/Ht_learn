...
def process_message(m):  # تعریف تابع برای پردازش پیام
    if "/" in m.text:
        word = m.text
        if "! " in word:
            ...
            try:
                ...
                data = {}
            data[word] = list(set(data.get(word, []) + answers))
        ...
        with open('data.json') as f:
            data = json.load(f)
        sorted_data = sorted(data.items(), key=lambda x: len(x[0]), reverse=True)
        hep = ''
        ...
        with open("data.json", "r") as json_file:
            data = json.load(json_file)
        if m.text in data and m.text not in ["بات", "عشقم", "هعی"]:
            random_answer = random.choice(data[m.text])
...
def read_data():
 with open('gang.json', 'r') as read:
  return load(read)
def write_data(data):
 with open('gang.json', 'w') as write:
